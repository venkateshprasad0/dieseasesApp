...
public class JohnHopkinsStrategy implements IndianDiseaseStat {
	...
	public String GetActiveCount() {
		...
		try {
			...
			JohnHopkinResponse[] johnHopkinResponse = new JohnHopkinResponse[0];
			johnHopkinResponse = getJohnHopkinResponses();
			...
			Float result = Arrays.stream(johnHopkinResponse).filter(x -> x.getCountry().equalsIgnoreCase("India"))
					.map(x->x.getStats().getConfirmed())
